...
const ProfileSetupScreen = ({ navigation }) => {
  ...
  const bmiInfo =
    heightCm && weightKg
      ? calculateBMI(parseFloat(weightKg), parseFloat(heightCm))
      : null;
  ...
  const validateStep = (currentStep) => {
    ...
    if (currentStep === 2) {
      ...
      if (!heightCm) newErrors.heightCm = 'Height is required';
        ...
        newErrors.heightCm = 'Enter a valid height (100–250 cm)';
      if (!weightKg) newErrors.weightKg = 'Weight is required';
      else if (isNaN(w) || w < 30 || w > 300)
        newErrors.weightKg = 'Enter a valid weight (30–300 kg)';
    }
    if (currentStep === 3) {
  ...
  const handleSave = async () => {
    ...
    if (result.success) {
      // Setup push notifications after profile is saved
      await setupNotifications(name.trim().split(' ')[0]);
      // AppNavigator will automatically navigate to Main based on auth state
    } else {
        ...
        <View
          style={[
            styles.inputContainer,
            {
              ...
              borderColor: errors.name ? COLORS.danger : (isDark ? COLORS.dark.border : COLORS.light.border),
            },
          ]}
        >
      ...
      <View style={styles.fieldGroup}>
        ...
        <View
          style={[
            ...
            {
              backgroundColor: isDark ? COLORS.dark.inputBg : COLORS.light.inputBg,
              borderColor: errors.age ? COLORS.danger : (isDark ? COLORS.dark.border : COLORS.light.border),
            },
          ]}
        >
          ...
          <TextInput
            value={age}
            onChangeText={(t) => { setAge(t.replace(/[^0-9]/g, '')); if (errors.age) setErrors((e) => ({ ...e, age: null })); }}
            placeholder="25"
            placeholderTextColor={colors.textMuted}
  ...
  const renderStep2 = () => (
    <View>
      ...
      <Text style={[styles.stepSubtitle, { color: colors.textSecondary }]}>
        Used for accurate calorie and BMI calculations
      </Text>
      ...
      <View style={styles.fieldGroup}>
        ...
        <View
          style={[
            ...
            {
              backgroundColor: isDark ? COLORS.dark.inputBg : COLORS.light.inputBg,
              borderColor: errors.heightCm ? COLORS.danger : (isDark ? COLORS.dark.border : COLORS.light.border),
            },
          ]}
        >
          ...
          <TextInput
            value={heightCm}
            onChangeText={(t) => { setHeightCm(t.replace(/[^0-9.]/g, '')); if (errors.heightCm) setErrors((e) => ({ ...e, heightCm: null })); }}
            placeholder="175"
            placeholderTextColor={colors.textMuted}
      ...
      <View style={styles.fieldGroup}>
        ...
        <View
          style={[
            ...
            {
              backgroundColor: isDark ? COLORS.dark.inputBg : COLORS.light.inputBg,
              borderColor: errors.weightKg ? COLORS.danger : (isDark ? COLORS.dark.border : COLORS.light.border),
            },
          ]}
        >
          ...
          <TextInput
            value={weightKg}
            onChangeText={(t) => { setWeightKg(t.replace(/[^0-9.]/g, '')); if (errors.weightKg) setErrors((e) => ({ ...e, weightKg: null })); }}
            placeholder="70"
            placeholderTextColor={colors.textMuted}
            keyboardType="decimal-pad"
            style={[styles.textInput, { color: colors.text }]}
  ...
  const renderStep3 = () => (
    <View>
      ...
      <Text style={[styles.stepSubtitle, { color: colors.textSecondary }]}>
        This shapes your AI-generated workout plans
      </Text>
      ...
      {errors.fitnessLevel && (
        <Text style={styles.fieldError}>{errors.fitnessLevel}</Text>
      )}
      <View style={styles.optionsGrid}>
        {FITNESS_LEVELS.map((level) => {
          ...
          return (
            <TouchableOpacity
              key={level.value}
              onPress={() => { setFitnessLevel(level.value); if (errors.fitnessLevel) setErrors((e) => ({ ...e, fitnessLevel: null })); }}
              style={[
                styles.optionCard,
                {
                  backgroundColor: isSelected ? `${COLORS.primary}18` : (isDark ? COLORS.dark.card : COLORS.light.card),
      ...
      {errors.fitnessGoal && (
        <Text style={styles.fieldError}>{errors.fitnessGoal}</Text>
      )}
      <View style={styles.goalsGrid}>
        {FITNESS_GOALS.map((goal) => {
          ...
          return (
            <TouchableOpacity
              key={goal.value}
              onPress={() => { setFitnessGoal(goal.value); if (errors.fitnessGoal) setErrors((e) => ({ ...e, fitnessGoal: null })); }}
              style={[
                styles.goalCard,
                {
                  backgroundColor: isSelected ? `${COLORS.primary}18` : (isDark ? COLORS.dark.card : COLORS.light.card),
            ...
            >
              ...
              <Text style={[styles.goalLabel, { color: isSelected ? COLORS.primary : colors.text }]}>
                {goal.label}
              </Text>
            </TouchableOpacity>
          );
